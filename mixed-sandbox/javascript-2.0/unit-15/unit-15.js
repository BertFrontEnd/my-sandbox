...
const f4 = () => {
  ...
  let out = Array.from(s4).some((el) => el === i4);
...
const f6 = () => {
  ...
  out6.textContent = new Set(a6).size;
};
...
const f7 = () => {
  ...
  if (arr7.length === set7.size && set7.size > 6) {
...
const f8 = () => {
  ar8.push(Array.from(s8).filter((el) => el > 5));
  console.log(ar8);
};
...
const f12 = () => {
  return new Set(str12.split('').filter((el) => el != ' '));
};
...
const f13 = () => {
  ...
  myArr.forEach((outEl) =>
    mySet.filter((innerEl) => {
      outEl === innerEl ? (myObj[outEl] = 1) : (myObj[outEl] += 1);
    }),
  );
